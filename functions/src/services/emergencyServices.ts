...
import { EmergencyNumbersResult } from '../types';

export const EMERGENCY_NUMBERS_DATABASE: Record<string, EmergencyNumbersResult> = {
  IN: {
    countryCode: 'IN',
    countryName: 'India',
    police: '100',
...
export function lookupEmergencyNumbers(countryCode?: string): EmergencyNumbersResult {
  const normalized = (countryCode || 'IN').trim().toUpperCase();
  if (EMERGENCY_NUMBERS_DATABASE[normalized]) {
    return EMERGENCY_NUMBERS_DATABASE[normalized];
  }

  // Check aliases or region mapping
  if (normalized === 'GB') return EMERGENCY_NUMBERS_DATABASE['UK'];
  if (normalized === 'USA') return EMERGENCY_NUMBERS_DATABASE['US'];
  if (normalized === 'IND') return EMERGENCY_NUMBERS_DATABASE['IN'];
