...
exports.getIncidentTypeMaxLifetimeMinutes = getIncidentTypeMaxLifetimeMinutes;
exports.computeDecayedConfidence = computeDecayedConfidence;
exports.checkCorroboration = checkCorroboration;
exports.computeAreaSafetyStatus = computeAreaSafetyStatus;
exports.scoreCandidateRoutes = scoreCandidateRoutes;
...
function getIncidentTypeMaxLifetimeMinutes(type) {
    switch (type) {
        case 'accident':
        case 'fire':
...
function computeDecayedConfidence(report, now = Date.now()) {
...
function checkCorroboration(newReport, existingReports) {
    ...
    for (const existing of existingReports) {
        if (existing.type !== newReport.type)
            continue;
        if (existing.status === 'expired')
            continue;
        const timeDiff = Math.abs(newReport.timestamp - existing.timestamp);
        if (timeDiff > CORROBORATION_TIME_WINDOW_MS)
            continue;
...
function computeAreaSafetyStatus(targetPoint, radiusMeters, nearbyIncidents, currentTime = new Date()) {
    ...
    for (const inc of nearbyIncidents) {
        ...
        if (confidence <= 0.1)
            continue; // ignore fully decayed reports
        activeCount++;
        ...
        summaries.push(`${inc.type.replace('_', ' ').toUpperCase()} reported ${ageMins}m ago (${dist.toFixed(0)}m away, confidence ${Math.round(confidence * 100)}%)`);
    }
    // 2. Time of day risk factor
    const hour = currentTime.getHours();
    let timeOfDayFactor = 1.0;
    let pedestrianActivity = 'moderate';
...
function scoreCandidateRoutes(routes, allIncidents, currentTime = new Date()) {
    return routes.map((route) => {
        ...
        let safetyLevel = 'green';
        if (rawSafetyScore < 60)
            safetyLevel = 'red';
        else if (rawSafetyScore < 80)
            safetyLevel = 'yellow';
        ...
        return {
            ...
            signals,
            recommendedReason: safetyLevel === 'green'
...
function checkRouteDeviation(userPoint, routeCoordinates, thresholdMeters = 150) {
...
//# sourceMappingURL=safetyPulseEngine.js.map